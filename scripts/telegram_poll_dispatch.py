...
동작:
  1. getUpdates 로 밀린 메시지와 블로그 버튼 클릭을 한 번에 읽는다
  2. /make [주제] · /daily · /help 또는 블로그 발행 버튼을 처리한다
...
def fetch_updates():
    """밀린 업데이트 원문과 마지막 update_id를 돌려준다."""
    url = f"{API}/getUpdates"
    res = requests.get(
        url,
        params={"timeout": 0, "limit": 100, "allowed_updates": json.dumps(["message", "callback_query"])},
        timeout=30,
    )
...
def answer_callback(callback_id, text, show_alert=False):
    try:
        ...
        print(f"[!] 버튼 응답 실패: {exc}")


def handle_blog_callback(callback):
    message = callback.get("message") or {}
    chat = str((message.get("chat") or {}).get("id", ""))
    callback_id = callback.get("id", "")
    data = (callback.get("data") or "").strip()
    if CHAT_ID and chat != CHAT_ID:
        answer_callback(callback_id, "허용되지 않은 대화방입니다.", show_alert=True)
        return
    if data.startswith("blog_hold:"):
        answer_callback(callback_id, "보류했습니다. 글은 Blogger 임시저장에 그대로 남아 있습니다.")
        return
    if not data.startswith("blog_publish:"):
        return

    post_id = data.split(":", 1)[1]
    if not post_id.isdigit():
        answer_callback(callback_id, "잘못된 글 번호입니다.", show_alert=True)
        return
    answer_callback(callback_id, "발행을 시작합니다…")
    try:
    ...
    if result.get("ok"):
        ...
        reply(f"✅ <b>[QA+] 공개 발행 완료</b>\n\n📌 {title}\n🔗 {url}")
        answer_callback(callback_id, "공개 발행이 완료됐습니다.")
    else:
        error = result.get("error", "알 수 없는 오류")
        reply(f"🚨 <b>[QA+] 발행 실패</b>\n\n{error[:400]}")
        answer_callback(callback_id, "발행에 실패했습니다. 안내 메시지를 확인하세요.", show_alert=True)
...
def main():
    ...
    updates, last_id = fetch_updates()
    if not updates:
        print("새 명령 또는 버튼 클릭 없음.")
        if last_id is not None:
            confirm_offset(last_id)
        return

    print(f"수신한 업데이트 {len(updates)}건")
    ...
    # 실행 전에 확정하여 네트워크 재시도 때 발행 버튼이 중복 처리되지 않게 한다.
    confirm_offset(last_id)

    for callback in callbacks:
        handle_blog_callback(callback)
