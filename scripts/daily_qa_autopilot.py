...
1. 켄 번스(Ken Burns) 카메라 줌인 & 슬림 HUD 카드로 실사 B-Roll 노출 극대화
2. 키네틱 강조 자막 및 골드/네온 배지
3. 신뢰형 BGM + 씬 전환 Whoosh + 팁 강조 Ding + 체크 Pop SFX 3중 오디오 믹싱
4. 실시간 HACCP 레이저 스캔 바 & 모션 라이브 센서 이펙트
5. 온도계 게이지 & 감도 캘리퍼 인포그래픽 모션 그래픽 탑재
"""
...
def generate_dynamic_scenes_for_custom_topic(topic_name):
    ...
    is_temp = any(k in topic_name for k in ["살균", "가열", "냉각", "온도", "Cold", "냉동"])
    is_metal = any(k in topic_name for k in ["금속", "이물", "X-ray", "엑스레이", "검출"])
    info_type = "temp" if is_temp else ("metal" if is_metal else "steps")

    return [
        {
            "id": 1, "badge": "🚨 실무 긴급 진단", "badge_color": (239, 68, 68),
            "title": f"HACCP 핵심 점검!\n{topic_name[:16]}",
            "subtitle": "20년 선배가 알려주는 3분 실무 가이드",
            "key_points": [
                f"{topic_name[:18]} 관련 법적 기준 준수",
                "심사관이 현장에서 불시 점검하는 핵심 포인트"
            ],
            "senior_tip": "현장 일지와 실제 작업 프로세스의 일치성 확인!",
            "infographic": info_type,
            "image": os.path.join(ASSETS_DIR, "broll_metal_line.jpg"),
...
def render_scene_frame(scene_data, frame_num_in_scene=0, total_scene_frames=300):
    ...
    # 1. 켄 번스(Ken Burns) 줌인 & 패닝 연산 (1.00x -> 1.15x)
    ...
    if os.path.exists(scene_data["image"]):
        ...
        crop_y = int((orig_h - crop_h) * (0.3 + 0.2 * progress)) # Smooth subtle vertical pan
    ...
    # 2. B-Roll 실사가 시원하게 보이도록 투명도/그라데이션 최적화 (배경 노출 대폭 확대)
    ...
    # 카드를 컴팩트하게 축소(Y1=560, Y2=1280)하여 배경 공장이 시원하게 보임
    ...
    info_type = scene_data.get("infographic", "temp")
    if info_type == "temp":
        # 온도계 게이지 모션 (0℃ -> 85℃ 상승)
...
def mix_scene_audio_with_sfx(tts_file, duration, scene_id):
    """TTS 음성에 씬 전환 Whoosh와 Pop 효과음을 합성"""
    ...
    # Mix: Whoosh at t=0, Pop at t=0.5, TTS voice at 1.0 volume
    filter_str = (
...
def run_daily_autopilot(custom_topic=None):
    ...
    # 0. Ensure audio assets (BGM & SFX) exist
    ...
    for idx, s in enumerate(scenes):
        ...
        # Mix SFX (Whoosh, Pop) with TTS
        mixed_scene_audio = mix_scene_audio_with_sfx(raw_audio, duration, s["id"])
        
        # Render dynamic keyframe series for smooth Ken Burns motion (every 0.25s = 1 keyframe, interpolated)
        # To keep rendering fast and lightweight under 15 seconds per scene, render multi-frame animation sequence:
        num_key_frames = 12
        ...
        # Encode looping keyframes sequence with smooth crossfade
        cmd = [
            "ffmpeg", "-y",
